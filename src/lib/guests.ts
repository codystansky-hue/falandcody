...
export function headcountFor(guests: Guest[], eventKey: string) {
  return guests
    .filter((g) => g.status !== 'no' && g.attending_events.includes(eventKey))
    .reduce((n, g) => n + headcount(g), 0)
}

export function tally(guests: Guest[]) {
  const replied = guests.length
  ...
  return {
    ...
    inBlock: guests.filter((g) => g.status !== 'no' && g.stay_pref === 'block').length,
    needHelp: guests.filter((g) => g.status !== 'no' && g.stay_pref === 'help').length,
    flying: guests.filter((g) => g.status !== 'no' && g.origin_airport).length,
...
export function groupTransfers(guests: Guest[], windowMinutes = 120) {
  const arriving = guests
    .filter((g) => g.needs_transfer && g.arrival_at && g.status !== 'no')
